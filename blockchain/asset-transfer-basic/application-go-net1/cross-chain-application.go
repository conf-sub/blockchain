...
func main() {
	...
	http.HandleFunc( "/makeRequest", func(w http.ResponseWriter, r *http.Request) {
		ip := "0.0.0.0"
		resp, err := http.PostForm("http://"+ip+":8060/fetchData", url.Values{"key": {"Value"}, "id": {"123"}})
		if err != nil {
			panic(err)
		}
	...
	http.HandleFunc( "/fetchData", func(w http.ResponseWriter, r *http.Request) {
		...
		resp, err := http.PostForm("http://0.0.0.0:8051/signMessage", url.Values{"num": {strconvItoa(numOrgs)}, "message": {string(bytes_mssg)}})
		if err != nil {
			panic(err)
		}
		defer resp.Body.Close()
